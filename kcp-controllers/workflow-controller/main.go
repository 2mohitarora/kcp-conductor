...
var (
	workflowDefGVK = schema.GroupVersionKind{
		Group: "workflows.com", Version: "v1alpha1", Kind: "WorkflowDefinition",
	}
	workflowRunGVK = schema.GroupVersionKind{
		Group: "workflows.com", Version: "v1alpha1", Kind: "WorkflowRun",
	}
)
...
func main() {
	...
	flag.StringVar(&apiExportName, "api-export-name", "workflow-specs", "Name of the APIExport")
	flag.StringVar(&conductorURL, "conductor-url", "http://conductor-server.conductor.svc.cluster.local:8080", "Conductor server URL")
...
func reconcileDefinition(
	...
	version := extractInt(conductorDef["version"], 1)
...
func pollExecution(
	...
	if err != nil {
		log.Error(err, "Failed to poll Conductor, retrying")
	...
	// Terminal state — done
	log.Info("Workflow execution finished", "phase", phase, "workflowId", workflowId)
	return reconcile.Result{}, nil
}
...
func extractInt(v interface{}, defaultVal int) int {
	switch n := v.(type) {
	case int:
		return n
	case int64:
